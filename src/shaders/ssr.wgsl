...
fn depth_texel_clamped(texel: vec2<i32>) -> f32 {
    ...
    return textureLoad(depth_texture, clamp(texel, vec2<i32>(0), max_coord), 0);
}
fn depth_nearest(uv: vec2<f32>, tex_size: vec2<f32>) -> f32 {
    let coord = uv * tex_size - vec2<f32>(0.5);
    return depth_texel_clamped(vec2<i32>(floor(coord + vec2<f32>(0.5))));
}
fn depth_bilinear(uv: vec2<f32>, tex_size: vec2<f32>) -> f32 {
    let coord = uv * tex_size - vec2<f32>(0.5);
...
fn evaluate_depth_distance(ray_point_ndc: vec3<f32>, depth_thickness: f32, tex_size: vec2<f32>) -> DistanceResult {
    let uv = ndc_to_uv(ray_point_ndc.xy);
    let ray_depth = 1.0 / ray_point_ndc.z;
    let linear_depth = 1.0 / depth_bilinear(uv, tex_size);
    let unfiltered_depth = 1.0 / depth_nearest(uv, tex_size);
...
fn trace_ray(ray_start_ndc: vec3<f32>, ray_end_ndc: vec3<f32>, jitter: f32, depth_thickness: f32) -> RayMarchHit {
    ...
    let step_count = clamp(u32(floor(length(ray_len_px))), 2u, params.max_steps);
    var min_t = 0.0;
    var max_t = 1.0;
...
@fragment fn fs(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let reflectivity = normal_sample.w;
    ...
    let view_normal = normal_sample.xyz / normal_len;
    ...
    ray_start_ndc += delta * max(0.0, max(dist_to_near.x, dist_to_near.y));
    ...
    delta *= min(min(dist_to_far.x, dist_to_far.y), dist_to_far.z);
    ray_end_ndc = ray_start_ndc + delta;
    let depth_thickness = params.thickness;
    let jitter = interleaved_gradient_noise(in.uv * params.screen_size);
    ...
    let reflected_color = min(textureSampleLevel(scene_color_texture, scene_sampler, hit_uv, 0.0).rgb, vec3<f32>(4.0));
    let fade_range = params.fade_end - params.fade_start;
    var distance_fade = 1.0;
    if fade_range > 0.001 {
        distance_fade = 1.0 - clamp((march.hit_t - params.fade_start) / fade_range, 0.0, 1.0);
    }
    let confidence = clamp(distance_fade * screen_edge_fade(hit_uv) * params.intensity * reflectivity, 0.0, 1.0);
