...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if visible && cull.occlusion_enabled != 0u {
        let lo = mesh_bounds.center - vec3<f32>(mesh_bounds.radius);
        let hi = mesh_bounds.center + vec3<f32>(mesh_bounds.radius);
    ...
    if visible {
        ...
        let first_instance = indirect[batch].first_instance;
        let write_index = atomicAdd(&indirect[batch].instance_count, 1u);
        visible_indices[first_instance + write_index] = index;
    }
}
